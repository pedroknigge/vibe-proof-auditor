...
- [ ] Pragmatic integration vs mythical perfection: the design favors cutting the system into verifiable, promptable parts and integrating them securely, rather than chasing a mythical, perfectly rigid "right" architecture that no startup actually builds. **Fail** if the architecture is over-engineered for theoretical purity instead of shipping a testable, modular, and integrated product. Finding id: `mythical-architecture-trap`.
...
- [ ] Balanced code generation: the codebase avoids blind "one-shot" vibe coding for the entire app, but also avoids writing every single piece of boilerplate manually. AI is used to implement clear architectural boundaries and proper unit/integration testing by parts. **Fail** if the codebase is an unverified single-shot dump without integration, or conversely, if the architecture actively fights automation by requiring excessive manual toil for every change. Finding id: `unbalanced-generation`.
